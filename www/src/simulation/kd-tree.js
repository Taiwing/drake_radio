export class KDTree {
  ...
  constructor(points,dimensions,this_dim=0){
    ...
    if (left.length > 0)
      self.left = new KDTree(left,dimensions,(this_dim+1) % self.ndim)
    else
      self.left = null
    ...
    // do that, creating a new tree that will split the points along the next dimension
    if (right.length > 0)
      self.right = new KDTree(right,dimensions,(this_dim+1) % self.ndim)
    else
      self.right = null
  }
